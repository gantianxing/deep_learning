...
vocabulary = read_data('天龙八部分词.txt')
...
with tf.Session(graph=graph) as session:
	...
	d = tf.reduce_sum(tf.square((final_embeddings-final_embeddings[dictionary['乔峰']])),1)
	_,n=tf.nn.top_k(-d,k=50)
	a = session.run(n)
	for i in range(50):
		print ("乔峰：",reverse_dictionary[a[i]])
